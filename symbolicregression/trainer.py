...
class Trainer(object):
    def __init__(self, modules, env, params, path=None, root=None):
        ...
        ### Load pretrained Encoder from SNIP
        if self.params.reload_model_snipenc != "":
            self.reload_model(modules_to_load=['embedder','encoder_y'], path= self.params.reload_model_snipenc, requires_grad=False)

        ## Load pretrained E2E decoder
